    ...
    if (request.method == "POST") {
      ...
      if(parseInt(contentLength) <= 5000) {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
          chrome.tabs.sendMessage(tabs[0].id, {captchaSolved: true}, function(response) {
          });
        })
      }   
    }
...
{urls: ["*://www.google.com/recaptcha/api2/userverify*","https://www.recaptcha.net/recaptcha/api2/userverify*"]},
["responseHeaders"])



chrome.webRequest.onCompleted.addListener(function(shipping) {
  console.log("Shipping loaded")
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        chrome.tabs.sendMessage(tabs[0].id, {shippingLoaded: true}, function(response) {
        });
      })
},
...
chrome.webRequest.onCompleted.addListener(function(queue) {
  if (queue.statusCode == 200) {
    console.log("Queue passed")
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      chrome.tabs.sendMessage(tabs[0].id, {queuePassed: true}, function(response) {
      });
    })
  } else {
...
chrome.webRequest.onCompleted.addListener(function(card) {

  if (card.method == "GET") {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      setTimeout(function() {
        chrome.tabs.sendMessage(tabs[0].id, {cardLoaded: true}, function(response) {
...
chrome.webRequest.onBeforeRequest.addListener(
  function(details) { 
    if (details.method == "POST") {
      var formData = decodeURIComponent(String.fromCharCode.apply(null,new Uint8Array(details.requestBody.raw[0].bytes)));
      chrome.storage.local.get({'nikeEnabled':"",'formdata':[]}, function(result) {
...
chrome.webRequest.onBeforeSendHeaders.addListener(
  function(details) {
      var token = details.requestHeaders[2]["value"]
      console.log(token)
      chrome.storage.local.set({'token': token})
  },
